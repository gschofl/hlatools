...
test_that("HLAAllele global API", {
  ...
  expect_equal(length(sequences(dpa)), 592)
...
test_that("HLAAllele element-wise API", {
  ...
  ##
  expect_equal(cwd_status(x), "")
  ##
  expect_equal(ancestry(x), "Asian - East Asian:European:Undefined")
  ##
  expect_equal(sample_name(x), "JM15:LB:LG2:MOU:PRIESS:QBL:T5-1:TUBO:WJR076")
  ##
  expect_true(is_complete(x))
  expect_false(is_lsl(x))
  ##
  expect_is(as(x, "data.table"), c("dtplyr_step_first", "dtplyr_step"))
})
...
test_that("HLAAllele subsetting", {
  ...
  expect_equal(NROW(dpa["01:03:01"]), 75)
  expect_equal(NROW(dpa["DPA1*01:03"]), 125)
  expect_equal(NROW(dpa["HLA-DPA1*01"]), 307)
